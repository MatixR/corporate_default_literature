...
google_search_expres <- expression({
  ...
  options(warn = 1)
  google_dat <- apply(
    review_data_subset[, c("Article name", "Publisher")], 1, function(x){
...
switch(
  ...
  update_all = {
    review_data_subset <- review_data
    
    eval(google_search_expres)
  ...
  update_new_or_missing = {
    cur_dat <- read.table("output/google_data", row.names = NULL, 
                          stringsAsFactors = F, header = T, allowEscapes = T)
    
    is_new <- which(!review_data$`Article name` %in% cur_dat$Article.name)
    is_all_missing <- which(apply(
      cur_dat[, !colnames(cur_dat) %in% c("Article.name", "Publisher")],
      1, function(x) all(is.na(x)))) 
    
    review_data_subset <- review_data[c(is_new, is_all_missing), ]
    cur_dat <- cur_dat[-is_all_missing, ]
    
    eval(google_search_expres)
    
    google_dat_after_eval <- google_dat
    tmp <- cbind(review_data_subset[, c("Article name", "Publisher")],
                 google_dat_after_eval)
    
    google_dat <- rbind(cur_dat, data.frame(tmp))
